...
export default function Register() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, background: "radial-gradient(ellipse at 50% 0%, rgba(34,211,238,0.06) 0%, var(--bg) 60%)" }}>
      ...
      <div className="card fade-up" style={{ width: "100%", maxWidth: 420 }}>
        <h1 style={{ fontFamily: "var(--font-head)", fontWeight: 700, fontSize: 24, marginBottom: 6 }}>Create account</h1>
        <p style={{ color: "var(--text-dim)", fontSize: 14, marginBottom: 28 }}>Start monitoring your servers in minutes</p>

        {/* Steps hint */}
        <div style={{ display: "flex", gap: 8, marginBottom: 28 }}>
          {["Create account", "Add your token", "Add servers", "Go live"].map((s, i) => (
            <div key={i} style={{ flex: 1, textAlign: "center" }}>
              <div style={{ height: 3, borderRadius: 2, background: i === 0 ? "var(--primary)" : "var(--border)", marginBottom: 6 }} />
              <div style={{ fontSize: 10, color: i === 0 ? "var(--primary)" : "var(--text-muted)" }}>{s}</div>
            </div>
          ))}
        </div>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            <label style={{ fontSize: 13, color: "var(--text-dim)", marginBottom: 6, display: "block" }}>Username</label>
